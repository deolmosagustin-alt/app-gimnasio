...
await page.route("**/api/ia", async (route) => {
  ...
  try {
    ...
    const sp = b.systemPrompt || "";
    const i = sp.indexOf('{"perfil"');
    const ctx = i >= 0 ? JSON.parse(sp.slice(i)) : {};
    const kb = (o) => Math.round(JSON.stringify(o || null).length / 1024 * 10) / 10;
    red.push({ totalKB: Math.round(sp.length/1024*10)/10, estaticoKB: i >= 0 ? Math.round(i/1024*10)/10 : null,
      desglose: { rutinas: kb(ctx.rutinas), logs: kb(ctx.logs), analisis: kb(ctx.analisisEntrenamiento), config: kb(ctx.configuracionActual), perfil: kb(ctx.perfil) },
      clavesDeLogs: Object.keys(ctx.logs || {}).length, entradasPorClave: Math.max(0, ...Object.values(ctx.logs || {}).map(v => Array.isArray(v) ? v.length : 0)), campos: Object.keys((Object.values(ctx.logs||{}).find(v=>Array.isArray(v)&&v.length)||[{}])[0]||{}).join(",") });
    await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ text: "ok", sources: [] }) });
  } catch (e) {
